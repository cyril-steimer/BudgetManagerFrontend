...
export class ReportComponent implements OnInit {
    ...
    private createContentConfig(includeTotal: boolean) {
        const res = [];
        for (const expense of this.expenses.getAllExpenses()) {
            res.push(new ContentConfig(expense.category.name, true, expense, false));
        }
        const total = this.expenses.getTotal();
    ...
    private createContent(): any {
        ...
        const summaryRows = Config.filterSelected(this.summaryRows);
        ...
        const detailTables = Config.filterSelected(this.detailTables);
    ...
    private createSummaryTable(rows: ContentConfig[]): any {
        const columns = Config.filterSelected(this.summaryColumns);
        const body = [this.applyStyleToAll(columns.map((col) => col.name), TH)];
        for (const row of rows) {
            const style = row.total ? TH : NONE;
    ...
    private createDetailsTable(expenses: CategoryExpenses): any {
        const columns = Config.filterSelected(this.detailColumns);
        const body = [this.applyStyleToAll(columns.map((col) => col.name), TH)];
        for (const expense of expenses.expenses) {
            const content = columns.map((col) => col.getColumnValue(expense));
            body.push(content);
        }
        return {
            widths: ['*', '*', '*', '*', '*'],
